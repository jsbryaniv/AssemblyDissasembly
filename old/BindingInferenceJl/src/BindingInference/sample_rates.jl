...
function sample_rates(data, variables; kwargs...)
    ...
    alphap = variables.k_micro_prop_shape
    ...
    function probability(k_bind_, k_micro_)

        # Prior
        prob = (
            sum(logpdf.(Gamma.(k_micro_shape[idp], k_micro_scale[idp]), k_micro_[idp]))
        )
        if k_bind_scale != 0
    ...
    # k_micro[end, idp0] .= rand(Dirichlet(k_micro_scale[end, idp0] .+ photo_init_counts[idp0]))
    for i = 1:num_micro
        for j = 1:num_micro
            k_micro_scale[i, j] <= 0 ? continue : nothing
            # Sample multiple times using Metropolis Hastings
            for _ in 1:10
                k_micro_old = copy(k_micro)
                k_micro_new = copy(k_micro_old)
                k_micro_new[i, j] = rand(Gamma(alphap, k_micro_old[i,j] / alphap))
                k_micro_new[i, i] -= sum(k_micro_new[i, :])
                accept_prob = (
                    probability(k_bind, k_micro_new)
                    - probability(k_bind, k_micro_old)
                    + logpdf(Gamma(alphap, k_micro_new[i, j] / alphap), k_micro_old[i, j])
                    - logpdf(Gamma(alphap, k_micro_old[i, j] / alphap), k_micro_new[i, j])
                )
        ...
        for i = 1:2
            for j = 1:2
                ...
                for _ in 1:10
                    ...
                    accept_prob = (
                        probability(k_bind_new, k_micro)
                        - probability(k_bind_old, k_micro)
